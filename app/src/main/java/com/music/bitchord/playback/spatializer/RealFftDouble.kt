...
/**
 * [RealFft] in double precision: the same algorithm with DoubleArray throughout. The convolver runs on it, so
 * its rounding error sits far below Float32's own (see [PartitionedConvolver]); the upmixer keeps [RealFft].
 *
 * Real-input FFT of a power-of-two length [n], allocation-free after construction.
 *
...
class RealFftDouble(val n: Int) {
    ...
    private val cosM = DoubleArray(m / 2)
    private val sinM = DoubleArray(m / 2)
    private val cosN = DoubleArray(m + 1)
    ...
    init {
        ...
        for (k in 0 until m / 2) {
            val a = -2.0 * PI * k / m
            cosM[k] = cos(a); sinM[k] = sin(a)
        }
        for (k in 0..m) {
            val a = -2.0 * PI * k / n
            cosN[k] = cos(a); sinN[k] = sin(a)
        }
    }
    ...
    fun forward(x: DoubleArray, re: DoubleArray, im: DoubleArray, offset: Int = 0) {
        ...
        for (k in 0..m) {
            ...
            val er = 0.5 * (ar + br); val ei = 0.5 * (ai + bi)
            // (a - b) / 2i = (ai - bi)/2 - i (ar - br)/2
            val odr = 0.5 * (ai - bi); val odi = -0.5 * (ar - br)
    ...
    fun inverse(re: DoubleArray, im: DoubleArray, out: DoubleArray, offset: Int = 0) {
        // E[k] = (X[k] + X*[m-k]) / 2,  O[k] = (X[k] - X*[m-k]) / (2 W^k),  Z = E + i O
        for (k in 0 until m) {
            val ar = re[k]; val ai = if (k == 0) 0.0 else im[k]
            val br = re[m - k]; val bi = -(if (m - k == m) 0.0 else im[m - k])
            val er = 0.5 * (ar + br); val ei = 0.5 * (ai + bi)
            val dr = 0.5 * (ar - br); val di = 0.5 * (ai - bi)
        ...
        for (j in 0 until m) {
            out[offset + 2 * j] = 2.0 * zr[j]
            out[offset + 2 * j + 1] = 2.0 * zi[j]
        }
    }
}
