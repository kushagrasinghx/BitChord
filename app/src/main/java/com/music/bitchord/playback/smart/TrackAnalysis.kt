...
data class TrackAnalysis(
    ...
    /**
     * Finetune Â§6.1: the transient fine energy curve, kept in memory only
     * (never persisted â€” see [AnalysisStore], which drops it on write). Lets
     * [buildupStart] re-derive the Â§4 gradient for analyses whose stored
    ...
    /**
     * Median fundamental over the head window's voiced frames, in Hz â€” the
    ...
    /**
     * v2 Â§2b: structural section labels over the full track, ascending.
     * Computed once by [StructureDetector] from the transient fine curves and
     * persisted â€” the fine curves themselves are never stored. Empty until a
     * whole-track analysis has run; callers fall back to energy heuristics.
     */
    val structureMap: List<StructureLabel> = emptyList(),
    /** v2 Â§2b: first DROP label start, else null (see `firstDropSec`). */
    val structuredDropSec: Double? = null,
    /** v2 Â§2b: first BREAK label start, else null. */
    val structuredBreakSec: Double? = null,
    /** v2 Â§2b: first OUTRO label start, else null. */
    val structuredOutroSec: Double? = null,
    /** v2 Â§4: Â§4-gradient buildup foot, else null (see `buildupStart`). */
    ...
    /** Phase A1: how the buildup foot was found â€” "gradient", "monotonic",
    ...
    /** Phase A1: drop âˆ’ foot in seconds, else null. */
    ...
    /**
     * Spec finetune Â§7: the track's own breathing room before the cut â€” the
    ...
    /**
     * Full-audit P0.2: true when this result came from the head-only pass â€”
     ...
     *
     * Full-audit Phase 2: deliberately no confidence gate here â€” a shaky grid
...
/** v2 Â§2b: one structural section label. Times are track-timeline seconds. */
data class StructureLabel(val start: Double, val end: Double, val type: StructureSectionType)

/** v2 Â§2b: the eight section kinds the detector emits, first-match-wins. */
...
data class TransitionPolicyVerdict(
    ...
    /** v2 Â§5a: harmonic tempo ratio locking bpmA onto bpmB (1.0 = unison). 1.0 by default. */
    val matchedRatio: Double = 1.0,
    /** Multi-candidate Â§Q2: semitone shift of the incoming key chosen by best-fit
...
enum class TransitionTier {
    ...
    /** v2 Â§1: tempi lock through a harmonic ratio (3:2, 4:3, half/double...),
     * not unison â€” both decks stretch to a shared BPM. Sits between
